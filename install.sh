...
post_install() {
    ...
    case "$PROFILE" in
        ...
        iot)
            ...
            echo "    # For ESP32-S3: cp firmware/esp32-csi-node/sdkconfig.defaults.esp32s3 firmware/esp32-csi-node/sdkconfig.defaults"
            echo "    # For ESP32-C3: cp firmware/esp32-csi-node/sdkconfig.defaults.esp32c3 firmware/esp32-csi-node/sdkconfig.defaults"
            echo "    # For ESP32-C6: cp firmware/esp32-csi-node/sdkconfig.defaults.esp32c6 firmware/esp32-csi-node/sdkconfig.defaults"
            echo "    # Edit sdkconfig.defaults: set SSID, password, aggregator IP"
            echo ""
            echo "    # 2. Build firmware (Docker — no local ESP-IDF needed):"
            echo "    # Replace 'esp32s3' with your target (esp32s3, esp32c3, esp32c6)"
            ...
            echo ""
            echo "    # 3. Flash to ESP32 (replace COM7 and --chip):"
            echo "    cd build && python -m esptool --port COM7 \\"
